...
## Jump passed the functions so you can see the main script.

# Naive reorient_surface
...
def get_abbott_stats(detrended_surface, imageHeight, imageWidth):
    ...
    return (round(sk[0],2), round(spk[0],2), round(svk[0],2), round(ratio_spk_sk,2), round(ratio_svk_sk,2))
...
def extract_surface_stats(surface_im):
    ...
    # Plot mean
    plot_mean = round(np.mean(detrended_image),2)

    # Positive vals
    pos_vals_mean = round(np.mean(detrended_non_zero[detrended_non_zero > 0]),2)
    pos_vals_std = round(np.std(detrended_non_zero[detrended_non_zero > 0]),2)

    # Negative vals
    neg_vals_mean = round(abs(np.mean(detrended_non_zero[detrended_non_zero < 0])),2)
    neg_vals_std = round(np.std(detrended_non_zero[detrended_non_zero < 0]),2)

    #Arithmatical Mean Deviation
    amd = round((np.sum(abs(detrended_non_zero)) / non_zero_count),2)
    rms = round((np.sum(detrended_non_zero**2)/ non_zero_count)**0.5,2)

    # Calculate roughness statistics
    
    # Scipy skew, kurtosis
    skew = round(get_skew(detrended_non_zero),2)
    kurtosis = round(get_kurtosis(detrended_non_zero),2)
    ...
    return stats,stats_num, normalised_detrended
